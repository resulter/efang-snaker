    ...
    public void schedule(JobEntity entity) {
        ...
        switch (entity.getJobType()) {
            ...
            case 1:
                ...
                break;
            case 2:
                jobClazz = AutoTaskJob.class;
                jobId = TYPE_AUTOTASK + entity.getTask().getId();
                break;
            case 3:
                jobClazz = FayaoJob.class;
                jobId = TYPE_FAYAO + entity.getId();
                break;
        }
        if (jobClazz == null) {
        ...
        if (jobClazz == AutoTaskJob.class && entity.getPeriod() > 0) {
            int count = ConfigHelper.getNumerProperty(CONFIG_REPEAT);
            if (count <= 0) count = 1;
            builder.withSchedule(SimpleScheduleBuilder.
                    repeatMinutelyForTotalCount(count, entity.getPeriod()));
