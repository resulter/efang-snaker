 ...
 */
package com.efangtec.workflow.engine.access.jdbc;
...
public class BeanPropertyHandler<T> extends AbstractListHandler<T> {
	...
	protected T handleRow(ResultSet rs) throws SQLException {
		...
		for (int index = 1; index <= columnCount; index++) {
			 ...
			 * 根据字段索引index获取字段名称
			 */
			String column = com.efangtec.workflow.engine.access.jdbc.JdbcHelper.lookupColumnName(rsmd, index);
			/**
			...
			if (pd != null) {
				try {
					 ...
					 * 根据字段index、属性类型返回字段值
					 */
					Object value = com.efangtec.workflow.engine.access.jdbc.JdbcHelper.getResultSetValue(rs, index, pd.getPropertyType());
					try {
						/**
