 ...
 */
package com.efangtec.workflow.engine.access;

import com.efangtec.workflow.engine.DBAccess;
import com.efangtec.workflow.engine.SnakerException;
import com.efangtec.workflow.engine.access.dialect.Dialect;
import com.efangtec.workflow.engine.access.jdbc.JdbcHelper;
...
import com.efangtec.workflow.engine.helper.ConfigHelper;
...
public abstract class AbstractDBAccess implements DBAccess {
	...
	protected static final String ORDER_INSERT = "insert into wf_order (id,process_Id,creator,create_Time,parent_Id,parent_Node_Name,expire_Time,last_Update_Time,last_Updator,order_No,variable,version,creator_name) values (?,?,?,?,?,?,?,?,?,?,?,?,?)";
//	protected static final String ORDER_INSERT = "insert into wf_order (id,process_Id,creator,create_Time,parent_Id,parent_Node_Name,expire_Time,last_Update_Time,last_Updator,order_No,variable,version) values (?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	protected static final String ORDER_HISTORY_INSERT = "insert into wf_hist_order (id,process_Id,order_State,creator, creator_Name,create_Time,end_Time,parent_Id,expire_Time,order_No,variable) values (?,?,?,?,?,?,?,?,?,?,?)";
	...
	protected static final String TASK_ACTOR_HISTORY_INSERT = "insert into wf_hist_task_actor (task_Id, actor_Id,creator) values (?, ?, ?)";
	...
	public void saveOrder(Order order) {
		if(isORM()) {
			...
			Object[] args = new Object[]{order.getId(), order.getProcessId(), order.getCreator(), order.getCreateTime(), order.getParentId(), 
                    ...
                    order.getVariable(), order.getVersion(),order.getCreatorName()};
			int[] type = new int[]{Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR,
					Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.INTEGER,Types.VARCHAR};
	...
	public void saveTaskActor(TaskActor taskActor) {
        if(isORM()) {
            ...
            saveOrUpdate(buildMap(TASK_ACTOR_INSERT, new Object[]{taskActor.getTaskId(), taskActor.getActorId(),taskActor.getCreator() }, type));
        }
    }
	...
	public void saveHistory(HistoryOrder order) {
		if(isORM()) {
			...
			Object[] args = new Object[]{order.getId(), order.getProcessId(), order.getOrderState(), order.getCreator(),order.getCreatorName(),
	...
	public void saveHistory(HistoryTask task) {
		if(isORM()) {
            ...
            if(task.getActorIds() != null) {
                for(String actorId : task.getActorIds()) {
                    if(StringHelper.isEmpty(actorId)) continue;
                    saveOrUpdate(buildMap(TASK_ACTOR_HISTORY_INSERT, new Object[]{task.getId(), actorId, task.getOperator()}, new int[]{Types.VARCHAR, Types.VARCHAR,Types.VARCHAR}));
                }
            }
		}
	}
	...
	public List<WorkItem> getWorkItems(Page<WorkItem> page, QueryFilter filter) {
		...
		sql.append(" select distinct o.process_Id, t.order_Id, t.id as id, t.id as task_Id, p.display_Name as process_Name, p.instance_Url, o.parent_Id, o.creator, o.creator_Name, ");
